...
public class PermGreedyOffice {
    ...
    public PermGreedyOffice() {
	initModel();
    }

    private void initModel() {
	office = new OfficeModel();
	spots = office.getNewSpots();
	destinations = office.getNewDestinations();
    }
	...
	sb.append("\n\n---------TALLY TEST----------\n\n");
	sb.append("\n----------NUM RUNS - " + numRuns + "-------\n");
	sb.append("\n---------PERM v. GREEDY OFFICE MODEL (n = 150)----------\n\n");
	for(int j = 1; j <= numRuns; j++) {
	    ...
	    sb.append("   ||   GREEDY AVG COST >> " + fourDecimals.format(goMatchingCosts[0]));
	    sb.append("\nPERMUATION WORST COST >> " + fourDecimals.format(pmMatchingCosts[1]));
	...
	sb.append("\nGREEDY ONLINE WINS " + "\n\nAVERAGE >> " + goWins[0] + "\nWORST >> " + goWins[1] + "\nBEST >> " + goWins[2]);
	try {
	    FileWriter fstream = new FileWriter("parking_results/PERM_GREEDY_TALLY_OFFICE_RESULTS.txt");
	...
	PermGreedyOffice pgo = new PermGreedyOffice();
	pgo.test();
    }
}