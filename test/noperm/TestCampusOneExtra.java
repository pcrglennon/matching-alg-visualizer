...
public class TestCampusOneExtra {
	...
	sb.append("\n----------NUM RUNS - " + numRuns + "-------\n");
	sb.append("\n---------CAMPUS MODEL ONE ADDITIONAL SPACES(100 destinations, 117 spots)----------\n\n");
	for(int j = 1; j <= numRuns; j++) {
	    ...
	    //Determine Winners
	    determineWinners(mfMatchingCosts, na1MatchingCosts, na2MatchingCosts, goMatchingCosts);
...
sb.append("\n\n-----------------------AVERAGE BOTTLENECK COSTS:-----------------------\n\n");
	...
	sb.append("\n\nFOR CAMPUS ONE (17 additional spaces):\n");
	sb.append("\n\nMAX FLOW WINS " + "\n\nAVERAGE >> " + mfWins[0] + "\nWORST >> " + mfWins[1] + "\nBEST >> " + mfWins[2]);
	sb.append("\n\nNEW ALG 1 WINS " + "\n\nAVERAGE >> " + na1Wins[0] + "\nWORST >> " + na1Wins[1] + "\nBEST >> " + na1Wins[2]);
	sb.append("\n\nNEW ALG 2 WINS " + "\n\nAVERAGE >> " + na2Wins[0] + "\nWORST >> " + na2Wins[1] + "\nBEST >> " + na2Wins[2]);
	sb.append("\n\nGREEDY WINS " + "\n\nAVERAGE >> " + goWins[0] + "\nWORST >> " + goWins[1] + "\nBEST >> " + goWins[2]);
    ...
    private void determineWinners(double[] mfCosts, double[] na1Costs, double[] na2Costs, double[] goCosts) {
	determineWinner(mfCosts[0], na1Costs[0], na2Costs[0], goCosts[0], 0);
	determineWinner(mfCosts[1], na1Costs[1], na2Costs[1], goCosts[1], 1);
	determineWinner(mfCosts[2], na1Costs[2], na2Costs[2], goCosts[2], 2);
    }
     ...
     */
    private void determineWinner(double mfCost, double na1Cost, double na2Cost, double goCost, int type) {
	double avgWinner = Math.min(Math.min(mfCost, na1Cost), Math.min(na2Cost, goCost));
	if(avgWinner == mfCost)
	    mfWins[type]++;
