...
/**
 * This contains various methods for running some (but not all) algorithms
 * 
 * Totally useless for the actual testing, but used by the Visual Matching
 * application (VisualMatchWindow)
 *
 * Note for this class, and other classes: due to poor planning, the terms cost
 * and distance may be used interchangeably
 */
public class MatchingAlgorithms {
    ...
    /**
     * Randomly generates new request and server nodes with locations within the
     * range specified by DISTANCE_RANGE
     */
    ...
    /**
     * Matches each request node (in order) to the unmatched server node w/ the
     ...
     *
     * This algorithm was made just as an experiment, became the basis for the 
     * Greedy Threshold algorithms, which are actually tested
     */
    ...
    /**
     * Constructs every possible request-server match, chooses the match w/ 
     * minimum cost, rinse and repeat until all requests are matched
     */ 
    ...
    /**
     * Iteratively searches through each possible match generated by 
     * constructFullMatchingList(), chooses minimum
     */
    ...
    /**
     * Constructs every possible match between unmatched request and server nodes
     */
    ...
    /**
     * Called after a request-server match is made, to remove the matched nodes
     * from the lists of nodes that are yet to be matched
     */
